...
class Video extends React.Component {
  ...
  componentDidMount() {
    ...
    var $video = (this.$video = this.api.$video = ReactDOM.findDOMNode(
      this.refs.video
    ));
    // window.$video = $video;
    ...
    if (this.props.autoPlay && !this.seekbarUpdateTimer)
      this.seekbarUpdateInterval();
  }
  ...
  _metaDataLoaded(e) {
    ...
    if (
      ...
      typeof this.props.metaDataLoaded == 'function'
    ) {
  ...
  _progress(e) {
    ...
    this.setState({
      loadedProgress: total / this.$video.duration * 100,
    });
  }
  ...
  $getSubtitleTracksMenu() {
    ...
    for (let ii = 0; ii < this.$video.textTracks.length; ii++) {
      ...
      $menuItems.push(
        <li key={ii}>
          <button onClick={e => this._setSubtitle(ii)}>{track.label}</button>
        </li>
      );
    }
    this.subTitleMenu = (
  ...
  $getSource(sources) {
    ...
    for (var ii = 0; ii < sources.length; ii++) {
      ...
      $sources.push(<source src={src} type={type} key={ii} />);
    }
    return $sources;
  }
  ...
  render() {
    ...
    let vHeight =
      this.props.height || $video.videoHeight || $video.clientHeight;
    ...
    if (this.props.controlPanelStyle == 'fixed')
      wraperStyle.height = vHeight + 50 + 'px';
      ...
      <div className="r5-wraper" style={wraperStyle}>
        <video ref="video" {...options}>
          ...
          {subtitles && subtitles.length > 0
            ? this.$getSubtitleTracks(subtitles)
            : ''}
        </video>
        ...
        <div className="r5-content" style={contentWraperStyle}>
          {this.props.children}
        </div>
            ...
            <div className="r5-volume">
              ...
              >
                ...
                <input
                  ...
                  onChange={e => this._volume(e.target.value)}
                />
              </div>
            </div>
            <span className="r5-timecode">
              {this.state.currentTime + ' / ' + this.state.duration}
            </span>
            <div className="r5-pull-right">
              {this.$subTitleMenu || this.$getSubtitleTracksMenu() || ''}
              <button className="r5-fullscreen" onClick={this._fullscreen}>
